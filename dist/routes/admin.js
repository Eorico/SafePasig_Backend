...
adminRouter.post('/admin-login', async (req, res) => {
    ...
    if (!email || !password)
        return res.status(400).json({ success: false, messsage: "Missing fields" });
    ...
    if (!isMatch)
        return res.status(401).json({ success: false, message: "Invalid Credentials" });
    const token = jwt.sign({ id: admin._id, email: admin.email, role: admin.role }, process.env.JWT_SECRET, { expiresIn: '8h' });
...
adminRouter.get("/reports/data-chart", adminAuth, async (req, res) => {
    ...
    reports.forEach(r => {
        ...
        if (r.status === "True")
            dataCharts.weekly.trueReports[day]++;
        if (r.status === "False")
            dataCharts.weekly.trueReports[day]++;
        // kada buwan
        ...
        if (week < 4) {
            if (r.status === "True")
                dataCharts.monthly.trueReports[week]++;
            if (r.status === "Fake")
                dataCharts.monthly.trueReports[week]++;
        }
        // kada Taon
        ...
        if (r.status === "True")
            dataCharts.yearly.trueReports[month]++;
        if (r.status === "Fake")
            dataCharts.yearly.trueReports[month]++;
        res.json(dataCharts);
    });
...
adminRouter.put("/reports/:id/status", adminAuth, permit("admin", "superadmin"), async (req, res) => {
    const { id } = req.params;
...
adminRouter.delete("/reports/:id", adminAuth, permit("superadmin"), async (req, res) => {
    const { id } = req.params;
